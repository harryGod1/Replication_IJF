...
def forecast_exp():
    ...
    f2 = open(new_path + "/Replication_IJF/data/2259/forecast/test1315_forecast_exposure_dtsm_x_Deli.txt")
    ...
    for i in range(len(test_data)):
        ...
        test_LT.append(int(line_data[10]));
        test_NB.append(int(line_data[11]));
        test_SEQ.append(str(line_data[12]))
        test_Age.append(int(line_data[13]));
        test_vintage.append(int(line_data[14]));
        test_calendar.append((int(line_data[15])));
        test_Def.append(int(line_data[16]));
        test_Deli.append(int(line_data[17]));
    ...
    for i in range(len(test_credit)): 
        ...
        #b2.append(test_SEQ[i]) just for test if the result excluded the exception
            ...
            #b2.append(int(s_p3[17]))
        b2.append(test_Deli[i])
        current_deli.append(int(line_data[17]))
        ...
        if(str(new_seq) == str(last_seq) and is_default == 0 and int(test_Age[i]) < max_seqlen ):
            ...
            #print('Exception1!')
        elif(str(new_seq) == str(last_seq) and is_default == 1 and test_Def[i] == 0):
            del x2[len(x2)-1]
            del y2[len(y2)-1]
            del current_deli[len(current_deli)-1]
            #print('last one: ',final_data[len(final_data)-2])
    ...
    #DTSM+Lag12+Exposure+Forecast
    #set lag time and initial calendat
    lag = (prs+1)*12
    current_calendar = 201301
    last_calendar = 0
    ...
    for i in range(lag_time):
        ...
        real_exposure = 0    
        real_all = 0
        count_real_exposure_default = 0
        for j in range(len(seqlen)):
            required_max_len = (int(current_calendar/100)*12 + int(current_calendar%100)*3) - (int(x2[seqlen_id][11]/100)*12 + int(x2[seqlen_id][11]%100)*3) + 3
            #print(required_max_len)
            #print(seqlen[j])
            if(seqlen[j] <= required_max_len - 3):
                1
            elif(seqlen[j] <= required_max_len):
                unconditional_pd = 1
                for k in range(seqlen[j]-1):
                    unconditional_pd = unconditional_pd * (1 - exposure_pd[seqlen_id+k])
                    #real exposure
                ead = x2[seqlen_id+seqlen[j]-1][3]*((1-((1+x2[seqlen_id+seqlen[j]-1][5])**(1/12))**(seqlen[j]-1-x2[seqlen_id+seqlen[j]-1][8]))/(1-((1+x2[seqlen_id+seqlen[j]-1][5])**(1/12))**(-x2[seqlen_id+seqlen[j]-1][8])))
                real_exposure = real_exposure + y2[seqlen_id+seqlen[j]-1]*0.3*ead

                if(y2[seqlen_id+seqlen[j]-1] == 1):
                    count_real_exposure_default += 1

                #predicted exposure    
                unconditional_pd = unconditional_pd * exposure_pd[seqlen_id+seqlen[j]-1]
                ead = x2[seqlen_id+seqlen[j]-1][3]*((1-((1+x2[seqlen_id+seqlen[j]-1][5])**(1/12))**(k-x2[seqlen_id+seqlen[j]-1][8]))/(1-((1+x2[seqlen_id+seqlen[j]-1][5])**(1/12))**(-x2[seqlen_id+seqlen[j]-1][8])))
                expected_all = expected_all + unconditional_pd*0.3*ead
                ...
                unconditional_pd = 1
                for k in range(required_max_len-1):
                    unconditional_pd = unconditional_pd * (1 - exposure_pd[seqlen_id+k])


                #predicted exposure    
                unconditional_pd = unconditional_pd * exposure_pd[seqlen_id+required_max_len-1]
                ead = x2[seqlen_id+required_max_len-1][3]*((1-((1+x2[seqlen_id+required_max_len-1][5])**(1/12))**(k-x2[seqlen_id+required_max_len-1][8]))/(1-((1+x2[seqlen_id+required_max_len-1][5])**(1/12))**(-x2[seqlen_id+required_max_len-1][8])))
                expected_all = expected_all + unconditional_pd*0.3*ead



            seqlen_id = seqlen_id + seqlen[j]

            real_all = real_all + real_exposure 
            real_exposure = 0

        last_calendar = current_calendar
        if((int(current_calendar%100) + 1)*3 > 12):
            current_calendar = (int(current_calendar/100) + 1)*100 + 1
        else:
            current_calendar = (int(current_calendar/100))*100 + int(current_calendar%100) + 1


        print(last_calendar,':')
        print('expected exposure for DTSM:',expected_all/len(seqlen))
        print('real exposure:',real_all/len(seqlen))
        print('============================================================')
            
    return real_all/len(seqlen),expected_all/len(seqlen)
...
for prs in range(0,3):
    ...
    if(data_mod == 1):
        ...
        #f2 = open("C:/Users/WH/Replication_IJF/data/2259/lag12_dtsm_train_all_HPI_UNRATE.txt")
        ...
        for i in range(len(test_data)):
            ...
            test_Age.append(int(line_data[14]));
            test_vintage.append(int(line_data[15]));
            test_calendar.append((int(line_data[16])));
            test_Def.append(int(line_data[18]));
            test_Deli.append(int(line_data[19]));
        ...
        for i in range(len(test_credit)):
            ...
            current_deli.append(int(line_data[19]))
            final_data.append(line_data)
            #b2.append(test_Deli[i])
            y.append(test_Def[i])
            x.append(b2)
    ...
    ct = ColumnTransformer(
        [('one_hot_encoder', OneHotEncoder(handle_unknown='ignore'), [1,6,7,11,12,13])],   # The column numbers to be transformed (here is [0] but can be [0, 1, 3])
    ...
    #f2 = open("D:/PhD_ml&cd/IJF_dataset/second_round/in_time/lag3_12_dtsm2_test04_1_HPI_UNRATE.txt")
    path = os.getcwd()
    new_path = path.replace("\\","/")
    f2 = open(new_path + "/Replication_IJF/data/2259/forecast/lag" + str(int(prs+1)*12) + "_dtsm_test_forecast1315_HPI_UNRATE.txt")
    ...
    for i in range(len(test_data)):
        ...
        test_vintage.append(int(line_data[15]));
        test_calendar.append((int(line_data[16])));
        test_Def.append(int(line_data[17]));
        test_Deli.append(int(line_data[18]));
    ...
    for i in range(len(test_credit)): 
        ...
        #b2.append(test_SEQ[i]) just for test if the result excluded the exception
            ...
            #b2.append(int(s_p3[16]))
        b2.append(test_Deli[i])
        current_deli.append(int(line_data[18]))
        final_data.append(line_data)
        y2.append(test_Def[i])
        x2.append(b2)
    ...
    #llr ratior test
    llr_test_unbalanced_dtsm_mev = unbalanced_log_likeli_dtsm
    ...
    print("DTSM Forecasting...")
    from sklearn import preprocessing
    #data for forecast
    path = os.getcwd()
    new_path = path.replace("\\","/")
    f1 = open(new_path + "/Replication_IJF/data/2259/forecast/lag" + str(int(prs+1)*12) + "_dtsm_test_forecast1315_HPI_UNRATE.txt")
    ...
    while train_data:
        ...
        train_vintage.append(int(line_data[15]));
        train_calendar.append(int(line_data[16]))
    ...
    for i in range(len(train_vintage)):

        b.append(int(train_vintage[i]))
    ...
    for i in range(lag_time):
        #
        seqlen_id = 0
        count_default = 0 # actual defaults within the lag period
        count_within_lag = 0 # actual observations within the lag period
        predicted_default = 0
        for j in range(len(seqlen)):
            required_max_len = (int(current_calendar/100)*12 + int(current_calendar%100)*3) - (int(account_vintage[seqlen_id][0]/100)*12 + int(account_vintage[seqlen_id][0]%100)*3) + 3
            #print(required_max_len)
            #print(seqlen[j])
            if(seqlen[j] <= required_max_len - 3):
                1
            elif(seqlen[j] <= required_max_len):
                for k in range(required_max_len - 3,seqlen[j]):
                    predicted_default = predicted_default + unbalanced_prediction[seqlen_id + k]
                    count_within_lag += 1
                    if(y2[seqlen_id + k] == 1.0):
                        count_default += 1
            else:
                for k in range(required_max_len - 3,required_max_len):
                    predicted_default = predicted_default + unbalanced_prediction[seqlen_id + k]
                    count_within_lag = count_within_lag + 1
                    if(y2[seqlen_id + k] == 1.0):
                        count_default += 1
            seqlen_id = seqlen_id + seqlen[j]
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/dtsm_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
    ...
    path = os.getcwd()
    new_path = path.replace("\\","/")
    f2 = open(new_path + "/Replication_IJF/data/2259/lag" + str(int(prs+1)*12) + "_dtsm_train_all_HPI_UNRATE.txt")
    ...
    for i in range(len(test_data)):
        ...
        test_UNRATE.append(float(line_data[13]))
        test_Age.append(int(line_data[14]));
        test_vintage.append(vt.transform([int(line_data[15])])[0]-1);
        test_calendar.append((int(line_data[16])));
        test_Def.append(int(line_data[17]));
        test_Deli.append(int(line_data[18]));
    ...
    y = np.array(y)
    #model = LogisticRegression(solver='newton-cg',penalty='l2',C=0.1).fit(x, y)
    #gam = LogisticGAM(f(1)+f(6)+f(7)+s(11)+f(12),lam = 1, n_splines = 25,spline_order = 2, fit_intercept = True).fit(x,y)
    #gam = LogisticGAM(l(0)+f(1)+l(1)+l(2)+l(3)+l(4)+l(5)+f(6)+f(7)+l(8)+l(9)+l(10)+s(11,n_splines=25,spline_order = 2)+f(12),lam = 1,fit_intercept = True).fit(x,y)
    gam = LogisticGAM(l(0)+f(1)+l(2)+l(3)+l(4)+l(5)+f(6)+f(7)+l(8)+l(9)+l(10)+s(11,n_splines=35,spline_order = 2)+f(12),lam = 1,fit_intercept = True).fit(x,y)
    #gam = LogisticGAM(l(0)+f(1)+l(1)+l(2)+l(3)+l(4)+l(5)+f(6)+f(7)+l(8)+l(9)+l(10)+s(11)+f(12),fit_intercept = True)
    ...
    x2 = []
    #y2 = [] #for forecasting, need to comment this line,otherwise, y2 will be cleared!!!!!!
    b2 = []
    ...
    for i in range(len(test_vintage)):

        b2.append(int(test_vintage[i]))
        b2.append(int(test_calendar[i]))
        x2.append(b2)
        b2=[]
    #print(x[0])
    #x = np.array(x)
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/gam_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
        reader = csv.reader(csvFile)
    ...
    print("Cox and Weibull...")

    #whether to open the calculation of exposure
    is_exposure_cph = False
    is_exposure_wb = False
    ...
    path = os.getcwd()
    new_path = path.replace("\\","/")
    f2 = open(new_path + "/Replication_IJF/data/2259/forecast/continuous_survival_train_account.txt")
    ...
    path = os.getcwd()
    new_path = path.replace("\\","/")
    f2 = open(new_path + "/Replication_IJF/data/2259/forecast/continuous_survival_test_account.txt")
    #f2 = open("./test4_data.txt")
    #f2 = open("./test3.txt")
    #train_data = f1.readline()
    #test_data = f2.readline()
    ...
    print("Forecasts of default rate:Cox...")
    ...
    while train_data:
        ...
        train_vintage.append(int(line_data[15]));
        train_calendar.append(int(line_data[16]))
        train_Def.append(int(line_data[17]))
    ...
    for i in range(lag_time):
        ...
        for j in range(len(seqlen)):
            ...
            if(seqlen[j] <= required_max_len - 3):
                ...
                for k in range(required_max_len - 3,seqlen[j]):
                    predicted_default = predicted_default + unbalanced_prediction_cph[seqlen_id + k]
                    count_within_lag += 1
                    if(y2[seqlen_id + k] == 1.0):
                        count_default += 1
            else:
    ...
    print("Forecasts of exposure at default:Cox...")

    #LSTM+Deli+Exposure+lag12

    exposure = 1

    exposure_pd = unbalanced_prediction_cph
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/cox_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
        reader = csv.reader(csvFile)
    ...
    for i in range(lag_time):
        ...
        for j in range(len(seqlen)):
            ...
            if(seqlen[j] <= required_max_len - 3):
                ...
                for k in range(required_max_len - 3,seqlen[j]):
                    predicted_default = predicted_default + unbalanced_prediction_wb[seqlen_id + k]
                    count_within_lag += 1
                    if(y2[seqlen_id + k] == 1.0):
                        count_default += 1
            else:
    ...
    print("Forecasts of exposure at default:Weibull...")

    #LSTM+Deli+Exposure+lag12

    exposure = 1

    exposure_pd = unbalanced_prediction_wb
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/weibull_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
        reader = csv.reader(csvFile)
    ...
    path = os.getcwd()
    new_path = path.replace("\\","/")
    f2 = open(new_path + "/Replication_IJF/data/2259/forecast/lag" + str(int(prs+1)*12) + "_dtsm_train_all_HPI_UNRATE.txt")
        ...
        def __init__(self, INPUT_FILE, win, all,discount,fixed_batch_size):
            ...
            for i in range(len(self.load_data)):
                ...
                if(str(new_seq) != str(last_seq)):
                    ...
                    t_indices.append(float(s[12]))
                    t_indices.append(float(s[13]))
                    ...
                    t_indices2.append(self.vt.transform([int(s[15])])[0])
    ...
    class BASE_RNN():
        ...
        def __init__(self,  EMB_DIM = 16,
            ...
            self.TRAIN_FILE = "D:/PhD_ml&cd/IJF_dataset/second_round/lag36_dtsm_train_all_HPI_UNRATE.txt"
            self.TEST_FILE = "D:/PhD_ml&cd/IJF_dataset/second_round/lag36_dtsm_test_all_HPI_UNRATE.txt"
        ...
        def create_graph(self):
            ...
            if self.add_time_feature:
                ...
                def add_time(x):
                    ...
                    y = tf.reshape(tf.tile(x[0:8], [self.MAX_SEQ_LEN]), [self.MAX_SEQ_LEN, self.FEATURE_SIZE2])
        ...
        def save_model(self):
            ...
            self.saver.save(self.sess, "D:/saved_model/model04to12_DeepHit_gpu_forecast_deli_noL2_512_16_exclude_exception/" + self.filename, global_step=self.global_step)
    ...
    FEATURE_SIZE2 = 8 # dataset input fields count
    ...
    path = os.getcwd()
    new_path = path.replace("\\","/")
    TEST_FILE = new_path + "/Replication_IJF/data/2259/forecast/lag" + str(int(prs+1)*12) + "_dtsm_test_forecast1315_HPI_UNRATE.txt"
    RUNNING_MODEL.test_data_win = SparseData(TEST_FILE, True, False,1, 512) 

    if(prs == 0):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_DeepHit_lag12_gpu_forecast_deli_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_DeepHit_lag12_gpu_forecast_deli_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    if(prs == 1):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_DeepHit_lag24_gpu_forecast_deli_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_DeepHit_lag24_gpu_forecast_deli_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    if(prs == 2):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_DeepHit_lag36_gpu_forecast_deli_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_DeepHit_lag36_gpu_forecast_deli_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    ...
    for i in range(0, int(RUNNING_MODEL.test_data_win.size / RUNNING_MODEL.BATCH_SIZE)):
        ...
        default = 0
        for i in range(len(predicted_label2)):
            default = default + predicted_label2[i][1]

        label_true = []
        for i in range(len(true_label2)):
            label_true.append(true_label2[i][1])

        prediction = []
        for i in range(len(predicted_label2)):
            prediction.append(predicted_label2[i][1])

        unbalanced_prediction2 = []
        for i in range(len(predicted_label2)):
            unbalanced_prediction2.append(0.1*predicted_label2[i][1]/(0.1*predicted_label2[i][1] - predicted_label2[i][1] + 1))

        unbalanced_default = 0
        for i in range(len(predicted_label2)):
            unbalanced_default = unbalanced_default + unbalanced_prediction2[i]

        unbalanced_log_likeli = 0
        for i in range(len(unbalanced_prediction2)):
            if(true_label2[i][0] == 1.0):
                unbalanced_log_likeli = unbalanced_log_likeli + math.log(1-unbalanced_prediction2[i])
            else:
                unbalanced_log_likeli = unbalanced_log_likeli + math.log(unbalanced_prediction2[i])
        ...
        #Forecasting
        prediction_DeepHit_deli = unbalanced_prediction2

        #process seqlen
        a = []
        for i in range(len(seqlen2)):
            if(int(i%2) == 0):
                a.append(int(seqlen2[i]))

        seqlen_DeepHit_deli = a
    ...
    prediction_DeepHit = unbalanced_prediction

    #process seqlen
    a = []
    for i in range(len(seqlen)):
        if(int(i%2) == 0):
            a.append(int(seqlen[i]))

    seqlen_DeepHit = a

    print("Forecasts of default rate:Deephit...")
    ...
    for i in range(lag_time):
        ...
        count_within_lag = 0 # actual observations within the lag period
        predicted_default = 0
        for j in range(len(seqlen_DeepHit)):
            required_max_len = (int(current_calendar/100)*12 + int(current_calendar%100)*3) - (int(account_vintage[seqlen_id][0]/100)*12 + int(account_vintage[seqlen_id][0]%100)*3) + 3
            #print(required_max_len)
            #print(seqlen[j])
            if(seqlen_DeepHit[j] <= required_max_len - 3):
                1
            elif(seqlen_DeepHit[j] <= required_max_len):
                for k in range(required_max_len - 3,seqlen_DeepHit[j]):
                    predicted_default = predicted_default + prediction_DeepHit[seqlen_id + k]
                    count_within_lag += 1
                    if(y2[seqlen_id + k] == 1.0):
                        count_default += 1
            else:
                for k in range(required_max_len - 3,required_max_len):
                    ...
                    if(y2[seqlen_id + k] == 1.0):
                        count_default += 1
            seqlen_id = seqlen_id + seqlen_DeepHit[j]
    ...
    print("Forecasts of exposure at default:Deephit...")



    exposure = 1

    exposure_pd = prediction_DeepHit

    seqlen = seqlen_DeepHit
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/deephit_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
        reader = csv.reader(csvFile)
    ...
    #3LSTM+Deli
    #version for lagged HPI&UNRATE + LSTM with Deli
    import tensorflow.compat.v1 as tf
    import numpy as np
    import sys
    from sklearn.metrics import roc_auc_score
        ...
        def __init__(self, INPUT_FILE, win, all,discount,fixed_batch_size):
            ...
            for i in range(len(self.load_data)):
                ...
                if(str(new_seq) != str(last_seq)):
                    ...
                    #if(max_seqlen ！= 0 and (max_seqlen+1 == len(current_deli)))        
                        #case for fist month is 1 not 0, manually add a 0 month for this account
    ...
    class BASE_RNN():
        ...
        def create_graph(self):
            ...
            if self.DNN_MODEL:
                ...
                rnn_cell = None
                rnn_cell = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.STATE_SIZE)
                rnn_cell1 = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.STATE_SIZE)
                rnn_cell2 = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.STATE_SIZE)
                rnn_cell3 = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.STATE_SIZE)
                #add dropout + MultiRNN
                ...
                outputs_Q, (h_c, h_n) = tf.nn.dynamic_rnn(
                    rnn_cell,                   # cell you have chosen
                    input_x,                    # input
                    initial_state=None,         # the initial hidden state
                    dtype=tf.float32,           # must given if set initial_state = None
                ...
                with tf.variable_scope("rnn1"):    
                    outputs_K, (h_c, h_n) = tf.nn.dynamic_rnn(
                        rnn_cell1,                   # cell you have chosen
                        input_x,                    # input
                        initial_state=None,         # the initial hidden state
                        dtype=tf.float32,           # must given if set initial_state = None
                        time_major=False,           # False: (batch, time step, input); True: (time step, batch, input)
                ...
                with tf.variable_scope("rnn2"):    
                    outputs_V, (h_c, h_n) = tf.nn.dynamic_rnn(
                        rnn_cell2,                   # cell you have chosen
                        input_x,                    # input
                        initial_state=None,         # the initial hidden state
                        dtype=tf.float32,           # must given if set initial_state = None
                        time_major=False,           # False: (batch, time step, input); True: (time step, batch, input)
                ...
                #e = tf.random.uniform([self.BATCH_SIZE,self.MAX_SEQ_LEN,self.MAX_SEQ_LEN],0.,5.)
                #self.attention = tf.nn.softmax(e,dim=-1)
                #self-dot with no prior weight
                e = tf.matmul(outputs_Q,tf.transpose(outputs_K,[0,2,1])) #(batch * time step * hidden size) * (batch * hidden size * time step) ---> batch * time step * time step
                self.attention = tf.nn.softmax(e,dim=-1) #batch * time step * time step
                self.attention_score_seq = tf.add(self.attention,tf.zeros([self.BATCH_SIZE,self.MAX_SEQ_LEN,self.MAX_SEQ_LEN]),name='attention_score_seq')
                final_outputs = tf.matmul(self.attention,outputs_V) # (batch * time step * time step) * (batch * time step * hidden size) ---> batch * time step * hidden size
                self.attention_weighted_output = final_outputs
                #outputs = tf.tanh(final_outputs)
                #outputs = tf.nn.dropout(final_outputs,0.9)

                self.attention_weighted_output = tf.add(final_outputs,tf.zeros([self.BATCH_SIZE,self.MAX_SEQ_LEN,self.STATE_SIZE]),name='attention_weighted_output')

                #final_outputs = tf.nn.dropout(final_outputs,0.5)
            ...
            for step in range(1, self.TRAING_STEPS + 1):
                ...
                if self.global_step < 300:
                    ...
                    if self.global_step % 300 == 0:
                        ...
                        self.run_test(sess)
                        self.save_model()
                elif self.global_step <= 60000:
                    self.SHOW_SURVIVAL_CURVE = True
                    if self.global_step % 500 == 0:
                        #1
                        self.run_test(sess)
                        self.save_model()
                elif self.global_step <= 80000:
                    self.SHOW_SURVIVAL_CURVE = True
                    if self.global_step % 1000 == 0:
                        1
                        #self.run_test(sess)
    ...
    #随机取值的时候，0那行会跟本体分开，单独成列，变成两笔账户。    
        ...
        def load(self, meta, ckpt, step):
            ...
            #self.train _op = graph.get_tensor_by_name("train_op:0")
            self.preds = graph.get_tensor_by_name("preds:0")
            self.attention_score_seq = graph.get_tensor_by_name("attention_score_seq:0")
        ...
        def run_test(self, sess):
            ...
            for i in range(0, int(self.test_data_win.size / self.BATCH_SIZE)):
                ...
                #test_batch_y = tf.ragged.RaggedTensorValue(test_batch_y,np.array([self.BATCH_SIZE]))
                start_time = time.time()
                bid_loss, bid_test_cross_entropy2, bid_true_label, bid_predicted_label,bid_weight,survival_rate,test_vintage= sess.run(
                    [self.cost,  self.cross_entropy2, self.true_label,self.predicted_label,self.weight,self.survival_rate,self.vintage],
                    feed_dict={self.tf_x: test_batch_x,
                                self.tf_x2: test_batch_x2,
                                self.tf_y: test_batch_y,
                                self.tf_y2: test_batch_y2,
    ...
    if(prs == 0):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_lag12_gpu_forecast_washout40_deli_random_3LSTM_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_lag12_gpu_forecast_washout40_deli_random_3LSTM_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    if(prs == 1):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_lag24_gpu_forecast_washout40_deli_random_3LSTM_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_lag24_gpu_forecast_washout40_deli_random_3LSTM_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    if(prs == 2):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_lag36_gpu_forecast_washout40_deli_random_3LSTM_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_lag36_gpu_forecast_washout40_deli_random_3LSTM_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    step = 100000
    ...
    for i in range(0, int(RUNNING_MODEL.test_data_win.size / RUNNING_MODEL.BATCH_SIZE)):
        ...
        ##PR-AUC
        #precision, recall, _ = precision_recall_curve(label_true, prediction)
        ...
        #Forecasting
        prediction_3LSTM_deli = unbalanced_prediction2
    ...
    for i in range(len(unbalanced_prediction)):
        if(true_label[i][0] == 1.0):
            unbalanced_log_likeli = unbalanced_log_likeli + math.log(1-unbalanced_prediction[i])
        else:
            unbalanced_log_likeli = unbalanced_log_likeli + math.log(unbalanced_prediction[i])
    ...
    for i in range(len(true_label)):
        ...
        if(true_label[i][0] == 1.0):
            ...
            log_good = log_good + math.log(predicted_label[i][0])

            count_good = count_good + 1
            mean_good = mean_good + predicted_label[i][1]
        if(true_label[i][0] == 0.0):
            bad.append(predicted_label[i][1])   
    ...
    prediction_3LSTM_deli = unbalanced_prediction
    ...
    print("Forecasts of default rate:3LSTM...")
    ...
    for i in range(lag_time):
        ...
        count_within_lag = 0 # actual observations within the lag period
        predicted_default = 0
        for j in range(len(seqlen_3LSTM_deli)):
            required_max_len = (int(current_calendar/100)*12 + int(current_calendar%100)*3) - (int(account_vintage[seqlen_id][0]/100)*12 + int(account_vintage[seqlen_id][0]%100)*3) + 3
            #print(required_max_len)
            #print(seqlen[j])
            if(seqlen_3LSTM_deli[j] <= required_max_len - 3):
                1
            elif(seqlen_3LSTM_deli[j] <= required_max_len):
                for k in range(required_max_len - 3,seqlen_3LSTM_deli[j]):
                    predicted_default = predicted_default + prediction_3LSTM_deli[seqlen_id + k]
                    count_within_lag += 1
                    if(label_true[seqlen_id + k] == 1.0):
                        count_default += 1
            else:
                for k in range(required_max_len - 3,required_max_len):
                    predicted_default = predicted_default + prediction_3LSTM_deli[seqlen_id + k]
                    ...
                    if(label_true[seqlen_id + k] == 1.0):
                        count_default += 1
            seqlen_id = seqlen_id + seqlen_3LSTM_deli[j]
    ...
    print("Forecasts of exposure at default:3lstm...")

    #3LSTM+Deli+Exposure+lag12

    exposure = 1

    exposure_pd = prediction_3LSTM_deli

    seqlen = seqlen_3LSTM_deli
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/3lstm_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
        reader = csv.reader(csvFile)
    ...
    class BASE_RNN():
        ...
        def __init__(self,  EMB_DIM = 16,
            ...
            self.INPUT_FILE = INPUT_FILE
            self.SAVE_LOG = SAVE_LOG
            self.TRAIN_FILE = "D:/PhD_ml&cd/IJF_dataset/second_round/lag36_dtsm_train_all16to21_HPI_UNRATE.txt"
            self.TEST_FILE = "D:/PhD_ml&cd/IJF_dataset/second_round/lag36_dtsm_test_all16to21_HPI_UNRATE.txt"
            self.OPEN_TEST = OPEN_TEST
            self.COV_SIZE = COV_SIZE
            self.SHOW_SURVIVAL_CURVE = False
            self.batch_seq = []
    ...
    if(prs == 0):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_lag12_gpu_forecast_washout40_deli_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_lag12_gpu_forecast_washout40_deli_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    if(prs == 1):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_lag24_gpu_forecast_washout40_deli_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_lag24_gpu_forecast_washout40_deli_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    if(prs == 2):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        meta = new_path + "/Replication_IJF/saved_model/model04to12_lag36_gpu_forecast_washout40_deli_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1.meta"
        ckpt = new_path + "/Replication_IJF/saved_model/model04to12_lag36_gpu_forecast_washout40_deli_attention seq selfdot_noL2_512_16" + "/drsa32_512_16_0.000100_0.100000_2259_1.20_0.20_True_False_1_1"
    step = 100000
    ...
    prediction_LSTM_deli = unbalanced_prediction
    ...
    print("Forecasts of default rate:LSTM+Deli...")
    ...
    for i in range(lag_time):
        ...
        count_within_lag = 0 # actual observations within the lag period
        predicted_default = 0
        for j in range(len(seqlen_LSTM_deli)):
            required_max_len = (int(current_calendar/100)*12 + int(current_calendar%100)*3) - (int(account_vintage[seqlen_id][0]/100)*12 + int(account_vintage[seqlen_id][0]%100)*3) + 3
            #print(required_max_len)
            #print(seqlen[j])
            if(seqlen_LSTM_deli[j] <= required_max_len - 3):
                1
            elif(seqlen_LSTM_deli[j] <= required_max_len):
                for k in range(required_max_len - 3,seqlen_LSTM_deli[j]):
                    predicted_default = predicted_default + prediction_LSTM_deli[seqlen_id + k]
                    count_within_lag += 1
                    if(label_true[seqlen_id + k] == 1.0):
                        count_default += 1
            else:
                for k in range(required_max_len - 3,required_max_len):
                    ...
                    if(label_true[seqlen_id + k] == 1.0):
                        count_default += 1
            seqlen_id = seqlen_id + seqlen_LSTM_deli[j]
    ...
    print("Forecasts of exposure at defualt:LSTM+Deli...")
    ...
    exposure_pd = prediction_LSTM_deli

    seqlen = seqlen_LSTM_deli
    ...
    if(exposure):
        ...
        csvFile = open(new_path + "/Replication_IJF/data/2259/lstm_forecast_exposure(22to24).csv", "r",encoding='gb18030', errors='ignore')
        reader = csv.reader(csvFile)
...
x_axis = []
hr = []
print('Forecasts Exposure at Default(2022-2024): ')
...
plt.title('Forecasts Exposure at Default(2022-2024)')
plt.xlabel('Time')
plt.show() 