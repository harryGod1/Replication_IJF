...
class SparseData():
    ...
    def next(self, batch_size):
        ...
        if self.batch_id + batch_size > len(self.data):
            ...
            #all_count_pre = 0
            #for i in range(0,len(self.data) - self.batch_id):
                #all_count_pre = all_count_pre + batch_seqlen[i] - 40
            #batch_labels2 = self.labels2[self.batch_all_id:self.batch_all_id+all_count_pre].tolist()
            #batch_deli = self.deli[self.batch_all_deli_id:self.batch_all_deli_id+all_count_pre].tolist()
...
class BASE_RNN():
    ...
    def __init__(self,  EMB_DIM = 16,
        ...
        self.TRAIN_FILE = new_path + "/Replication_IJF/data/" + INPUT_FILE + "/train_all_dtsm_x_forecast_Deli_random_debug0.txt"
        self.TEST_FILE = new_path + "/Replication_IJF/data/" + INPUT_FILE + "/test_all_dtsm_x_forecast_Deli_random_debug0.txt"
    ...
    def save_model(self):
        ...
        path = os.getcwd()
        new_path = path.replace("\\","/")
        self.saver.save(self.sess, new_path + "/Replication_IJF/saved_model/model04to12_gpu_new_forecast_washout40_deli_random_3Weighted LSTM_attention seq selfdot_noL2_512_16/" + self.filename, global_step=self.global_step)
