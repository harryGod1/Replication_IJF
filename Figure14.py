...
for prs in range(n_vintage):
    ...
    #lstm washout ablation 3lstm attention deli testing
    if(int(ym%100)<10):
        path = os.getcwd()
        new_path = path.replace("\\","/")
        TEST_FILE = new_path + "/Replication_IJF/data/2259/" + "test" + "0" + str(int(ym%100)) + "_" + str(q) + "_unbalanced_dtsm_x_Deli.txt"
    ...
    unbalanced_log_likeli_ablation_3lstm_attention_deli = 0
    for i in range(len(unbalanced_prediction)):
        if(true_label[i][0] == 1.0):
            unbalanced_log_likeli_ablation_3lstm_attention_deli = unbalanced_log_likeli_ablation_3lstm_attention_deli + math.log(1-unbalanced_prediction[i])
        else:
            unbalanced_log_likeli_ablation_3lstm_attention_deli = unbalanced_log_likeli_ablation_3lstm_attention_deli + math.log(unbalanced_prediction[i])
    ...
    #for log_likelihoo_ratio test
    llr_test_unbalanced_ablation_3lstm_attention_deli = unbalanced_log_likeli_ablation_3lstm_attention_deli 
    ...
    for m in range(len(time_window)): 
        ...
        for i in range(len(seqlen2)):
            ...
            if(seqlen2[i] <= time_window[m]):
                #1
                predict = unbalanced_prediction_ablation_3lstm_attention_deli[id:id+int(seqlen2[i])]
                h0 = predict[0]
                survival = 1-h0
                for j in range(1,len(predict)):
                    survival = survival*(1-predict[j])
            ...
            #max_age > time_window
                ...
                predict = unbalanced_prediction_ablation_3lstm_attention_deli[id:int(id+time_window[m])]
                h0 = predict[0]
                survival = 1-h0
                for j in range(1,len(predict)):
                    survival = survival*(1-predict[j])
    ...
    brier_score = 0
    for i in range(len(unbalanced_prediction_ablation_3lstm_attention_deli)):
        brier_score = brier_score + (unbalanced_prediction_ablation_3lstm_attention_deli[i]-int(true_label[i][1]))**2
    ...
    ########################################################################################################
    #index_g = prs
    #print('Pseudo-R-Square and AUC for each model in this round.')
    #print('As can be seen, our proposed model"3LSTMs based attention + Deli + washout" performs better than the model for ablation study.')
    #print("Pseudo_R-Square:")
    #print("3LSTMs based attention + Deli + washout|","3LSTM for ablation study|")
    #print(f"{(pseudo_3lstm[index_g]):.5f}","|",f"{(pseudo_3lstm_ablation[index_g]):.5f}")
    #print("AUC:")
    #print("3LSTMs based attention + Deli + washout|","3LSTM for ablation study|")
    #print(f"{(score_3lstm[index_g]):.5f}","|",f"{(score_3lstm_ablation[index_g]):.5f}")
        ...
        #plt.plot(x_axis,pseudo_3lstm,'blue',label='3LSTMs based attention + Deli + washout',)
        #plt.plot(x_axis,pseudo_3lstm_ablation,'red',label='unbalanced 3LSTMs based attention + Deli + washout',linestyle='--')
        ...
        #plt.plot(x_axis,score_3lstm,'blue',label='3LSTMs based attention + Deli + washout',)
        #plt.plot(x_axis,score_3lstm_ablation,'red',label='unbalanced 3LSTMs based attention + Deli + washout',linestyle='--')
