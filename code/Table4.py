...
class SparseData():
    ...
    def __init__(self, INPUT_FILE, win, all,discount,fixed_batch_size,phase):
        self.data = []
        self.data2 = []
        self.labels = []
        self.labels2 = []
        ...
        for i in range(len(load_data)):
            ...
            if(len(last_seq) ==  0):
                last_seq = s[11] 
            if(str(new_seq) == str(last_seq) and is_default == 0 and int(s[slen-4]) < max_seqlen ):
            ...
            if(str(new_seq) != str(last_seq)):
                ...
                self.seqlen.append(max_seqlen+1+self.phase)
        ...
        if(max_seqlen != 0):
            s = load_data[len(load_data)-1].split(' ')
            slen = len(s)
            t_indices = []
            t_indices2 = []
            #max_d = max(td,max_d)
    ...
    def next(self, batch_size):
        if self.batch_id + batch_size > len(self.data):
            ...
            for i in range(len(self.data)):
                ...
                for j in range(self.seqlen[i]-1-self.phase):
                    b.append([1., 0.])
        ...
        if(batch_size < self.fixed_batch_size):
            ...
            for i in range(self.fixed_batch_size - batch_size):
                ...
                batch_seqlen.append(1+self.phase) # Need add 40 to correspond with the washout when adding the behaviral & historical sequential variable
        ...
        for i in range(batch_size):
            all_count = all_count + batch_seqlen[i] - self.phase
...
class biSparseData():
    ...
    def next(self, batch):
        ...
        win = True
        if win:
            a, b, c, d, e= self.winData.next(batch)
...
class BASE_RNN():
    ...
    def __init__(self,  EMB_DIM = 16,
        ...
        self.TRAIN_FILE = "../data/" + INPUT_FILE + "/train_all_dtsm_x_forecast.txt"
        self.TEST_FILE = "../data/" + INPUT_FILE + "/test_all_dtsm_x_forecast.txt"
    ...
    def train_test(self,sess):
        ...
        for step in range(1, self.TRAING_STEPS + 1):
            ...
            batch_x, batch_x2, batch_y, batch_len, batch_y2, win = self.train_data.next(self.BATCH_SIZE)
    ...
    def run_test(self, sess):
        ...
        for i in range(0, int(self.test_data_win.size / self.BATCH_SIZE)):
            ...
            #
            test_batch_x, test_batch_x2, test_batch_y, test_batch_len, test_batch_y2 = self.test_data_win.next(
                self.BATCH_SIZE)
    ...
    def results(self,meta,ckpt,step):
...
###########################################################
#testing code block for reader to validate the results
#phase = 0
#for prs in range(1,5):
    #phase = prs*10
    ...
    #RUNNING_MODEL.test_data_win = SparseData(TEST_FILE, True, False,1, 512,phase)
        ...
        #nowashout phase use figure instead. Reader can train the nowashout model by themself and use this example code for testing.
        #path = os.getcwd()
        #new_path = path.replace("\\","/")
        #img = Image.open(new_path + 'Replication_IJF/data/2259/nowashout.png')
        #img.show()
        
    #else:
        #print("loading",'"',prs*10,"steps",'"', "model...")
        #path = os.getcwd()
        #new_path = path.replace("\\","/")
...
#print("The LSTM with 40 steps shows the highest AUC score and lowest -log-likelihood among the different washout phases")
#print("Washout Phase ","AUC ","-log-likelihood")
#print("10 steps: ",f"{(washout_AUC[0]):.5f}",f"{(washout_LLR[0]):.5f}")
